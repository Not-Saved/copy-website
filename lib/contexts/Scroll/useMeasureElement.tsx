...
export function useMeasureElement(ref: MutableRefObject<HTMLElement | SVGElement | null>) {
	...
	useLayoutEffect(() => {
		...
		if (ref.current) {
			...
			window.addEventListener("scroll-wrapper-resize", handleScrollWrapperResize);

			return () => {
				resizeObserver.disconnect();
				window.removeEventListener("scroll-wrapper-resize", handleScrollWrapperResize);
			};
		}
	}, [scrollY]);
...
function getMeasures(target: HTMLElement | SVGElement | Element, scrollY: number) {
